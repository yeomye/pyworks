...
class Customer:
    def __init__(self, cid, cname):
        self._cid = cid              # 고객 아이디(private - 접급제한)
        self._cname = cname          # 고객 이름(private - 접급제한)
    ...
    def showInfo(self):
        print(self.getname() + '님의 등급은 ' + self.cgrade + '이고, 보너스 포인트는' + str(self.bonus_point) + '점입니다.')

class GoldCustomer(Customer):
    def __init__(self, cid, cname):     # Customer 멤버(cid, cname) 상속
        super().__init__(cid, cname)
        self.cgrade = 'Gold'  # 고객 등급
        self.sale_ratio = 0.1   # 구매 할인율 - 10#
        self.bonus_ratio = 0.02  # 보너스 적립율 - 1%

    def calc_price(self, price):
        price = price - int(price * self.sale_ratio)    # 상품 가격 = 상품가격 - 할인된 가격
        self.bonus_point += int(price) * self.bonus_ratio  # 포인트는 정수로 변환
        return price
...
class VIPCustomer(Customer):
    def __init__(self, cid, cname,agent):
        ...
        self.sale_ratio = 0.1  # 구매 할인율 - 10#
        self.bonus_ratio = 0.05  # 보너스 적립율 - 1%
